    ...
    return (
      <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
        ...
        {weeks.map((week, weekIdx) => (
          <Box key={weekIdx} sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))' }}>
            {week.map((date, dayIdx) => {
              if (!date) {
                return <Box key={`empty-${dayIdx}`} sx={{ minHeight: 80, minWidth: 0, borderRight: '1px solid', borderBottom: '1px solid', borderColor: 'divider' }} />;
              }
