...
const ScheduleLibrary: React.FC<ScheduleLibraryProps> = ({
  ...
  const handleAddCommand = (isInlineEdit: boolean = false) => {
  ...
  const handleEditCommand = (index: number, isInlineEdit: boolean = false) => {
  ...
  const handleDeleteCommand = (index: number, isInlineEdit: boolean = false) => {
    if (isInlineEdit) {
  ...
  const handleCommandSave = () => {
    ...
    if (conflict) {
      alert('A command already exists at this time');
      return;
    }
      ...
      >
        ...
        <DialogContent>
          {viewingSpecificDates.length > 0 ? (
            <List>
              {viewingSpecificDates.map((date, index) => (
                <ListItem key={index}>
