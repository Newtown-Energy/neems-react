...
import { useEmergencyShutdownRequest } from '../utils/useEmergencyShutdownRequest';
...
const SldPage: React.FC = () => {
  ...
  const shutdown = useEmergencyShutdownRequest();
  ...
  return (
    <Box sx={{ p: 3, overflow: 'auto', flex: 1, minWidth: 0 }}>
      ...
      {/* A request recorded but not yet sent to the site. Deliberately not
          styled as an active E-stop: a request is not one. */}
      {!eStopActive && shutdown.pending && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Emergency shutdown requested — sending the signal to the site.
        </Alert>
      )}

      {/* The signal reached the RTAC. That is as far as this system can follow
          it: what the site did with it is not reported back, and the E-stop
          (alarm 104) is not its outcome. So say what is known, and no more. */}
      {!eStopActive && shutdown.recentlySent && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Emergency shutdown signal delivered to the site. This interface cannot
          see whether the site has shut down — confirm with on-site personnel.
        </Alert>
      )}

      {shutdown.failure && !eStopActive && (
        <Alert severity="error" sx={{ mb: 2 }}>
          Emergency shutdown signal could not be delivered: {shutdown.failure}.
          The site was
      ...
      {shutdown.error && (
        <Alert severity="error" sx={{ mb: 2 }} onClose={shutdown.dismissError}>
          {shutdown.error}
        </Alert>
      )}
      ...
      >
        <SingleLineDiagram onStateChange={setDiagramState} emergencyShutdown={shutdown} />
      </Box>
    </Box>
  );
};
